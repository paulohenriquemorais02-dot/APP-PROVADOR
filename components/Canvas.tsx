...
import { RotateCcwIcon, ChevronLeftIcon, ChevronRightIcon, BookmarkIcon } from './icons';
...
interface CanvasProps {
  ...
  onSaveLook?: () => void; // New prop for saving current look
}

const Canvas: React.FC<CanvasProps> = ({ displayImageUrl, onStartOver, isLoading, loadingMessage, onSelectPose, poseInstructions, currentPoseIndex, availablePoseKeys, onSaveLook }) => {
  const [isPoseMenuOpen, setIsPoseMenuOpen] = useState(false);
  const [showSaveConfirmation, setShowSaveConfirmation] = useState(false); // New state for save confirmation
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      ...
      {/* Save Look Button */}
      {displayImageUrl && !isLoading && onSaveLook && (
          <div className="absolute top-16 left-4 z-30 flex flex-col items-start">
              <button 
              ...
              <AnimatePresence>
                {showSaveConfirmation && (
                  <motion.div
                    ...
                    className="mt-2 px-3 py-1 bg-green-500 text-white text-xs rounded-full shadow-md"
                  >
                    Look Salvo!
                  </motion.div>
                )}
